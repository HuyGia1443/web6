    ...
    <section className='container mx-auto p-6'>
      ...
      <div className="grid grid-cols-4">
        ...
        return (
          ...
          <img src="https://picsum.photos/200" alt="" className='w-full object-cover rounded-tl-lg'/>
          <p className='text-lg font-semibold mt-5'>{product.productName}</p>
          <p>{product.price} vnd</p>
          <Link to={`/product-detail/${product.id}`} state={{product}} className='w-full  py-1 px-3 bg-green-500'>Chi tiet san pham</Link>
          <button className='w-full py-1 px-3 bg-blue-500'>Them san pham</button>
        </div>
        )
        })}
      </div>
